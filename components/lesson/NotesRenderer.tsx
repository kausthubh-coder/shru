...
import { parseNotesYaml, BlockT, CustomBlockT } from "@/types/notesYaml";

type NotesRendererProps = {
  yaml: string;
};

export function NotesRenderer({ yaml }: NotesRendererProps) {
...
function QuizBlockView({
  ...
  return (
    <div className="rounded-lg border border-white/10 bg-slate-800/60 p-4 md:p-5 w-full md:max-w-xl mx-auto">
      <div className="text-sm font-semibold mb-2 text-slate-50">
        {title ?? "Quiz"}
      </div>
            ...
            {submitted && (
              ...
              >
                {answers[q.id] === q.answer ? (
                  ...
                  <>Incorrect. Answer: <MathText text={q.answer} /></>
                )}
                {q.explanation ? (
      ...
      <div className="mt-3 flex items-center gap-2">
        <button
          className="text-xs px-2 py-1 rounded border border-emerald-400/30 bg-gradient-to-r from-emerald-600 to-cyan-600 hover:from-emerald-500 hover:to-cyan-500 text-white"
...
function InputBlockView({
  ...
  return (
    <div className="flex items-center gap-2 w-full md:max-w-xl mx-auto">
      <label htmlFor={id} className="text-sm min-w-[8rem] text-slate-200">
        {label}
      </label>
      <input
        id={id}
...
function EmbedBlockView({
  ...
  id: string;
  provider: "codepen" | "stackblitz" | "jsfiddle";
  refId: string;
  height: number;
}) {
  const src = useMemo(() => {
    if (provider === "codepen") {
      return `https://codepen.io/pen/embed/${refId}?default-tab=result`;
    }
    if (provider === "stackblitz") {
      return `https://stackblitz.com/edit/${refId}?embed=1&file=index.tsx`;
    }
    if (provider === "jsfiddle") {
      return `https://jsfiddle.net/${refId}/embedded/result/`;
    }
    return "about:blank";
  }, [provider, refId]);
